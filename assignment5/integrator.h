...
public:
    ...
    static dvec2 RK4(const VectorField2& vectorField, dvec2 position, double stepSize, const bool backward, bool normalize, double min_vel);
    
    static dvec2 Euler(const VectorField2& vectorField, const dvec2& position, const double stepSize);

    static std::vector<dvec2> integrateLine(   const dvec2 startPoint,
                                        double stepSize,
                                        const int numSteps, 
                                        const VectorField2 & vectorField, 
                                        const bool backward_line, 
                                        const bool inDirectionField, 
                                        double minVelocity, 
                                        const double maxArc);
   
    static bool outsideBoundary(const dvec2 & point, const dvec2 & min, const dvec2 & max );
};
